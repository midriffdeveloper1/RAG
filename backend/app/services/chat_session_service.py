...
class ChatSessionService:
    ...
    def list_sessions(self, browser_id: str) -> list[ChatSession]:
        return (
            self.db.query(ChatSession)
            .filter(ChatSession.browser_id == browser_id)
            .order_by(ChatSession.updated_at.desc())
            .all()
        )

    def get_session(self, session_id: str, browser_id: str) -> ChatSession | None:
        return (
            self.db.query(ChatSession)
            .filter(ChatSession.id == session_id, ChatSession.browser_id == browser_id)
            .first()
    ...
    def delete_session(self, session_id: str, browser_id: str) -> bool:
        session = self.get_session(session_id, browser_id)
        if session is None:
            return False
